...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *Genesis, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis, nil, engine, vmConfig, nil, nil)
...
func TestExtendCanonicalBlocksWithVmTracer(t *testing.T) {
	testCase := []vm.Config{
		{Tracer: logger.NewMarkdownLogger(&logger.Config{}, os.Stderr), Debug: true},
		{},
		{},
		{Tracer: logger.NewMarkdownLogger(&logger.Config{}, os.Stderr), Debug: true},
	}
	for i, tc := range testCase {
		...
		endTime := time.Now()
		// Shell command to show log:
		// go test -timeout 30m -run ^TestExtendCanonicalBlocksWithVmTracer$ github.com/ethereum/go-ethereum/core -test.v
		t.Logf("Run case %+v vmConfig=%+v with duration %+vms", i, vmConfig, endTime.UnixMilli()-startTime.UnixMilli())
	}
}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(blockchain.db, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
...
func TestLogReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
	defer blockchain.Stop()
...
func TestLogRebirth(t *testing.T) {
	var (
		...
		blockchain, _ = NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, engine, vmConfig, nil, nil)
	)
	defer blockchain.Stop()
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis, nil, engine, vmConfig, nil, nil)
	if err != nil {
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, nil)
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	chain, err := NewBlockChain(chaindb, nil, genesis, nil, engine, vmConfig, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func TestTransactionIndices(t *testing.T) {
	...
	for _, l := range limit {
		...
		l := l
		chain, err := NewBlockChain(ancientDb, nil, gspec, nil, ethash.NewFaker(), vmConfig, nil, &l)
		if err != nil {
			t.Fatalf("failed to create tester chain: %v", err)
		}
...
func TestTxIndexer(t *testing.T) {
	...
	for _, c := range cases {
		...
		chain, _ := NewBlockChain(db, nil, gspec, nil, engine, vmConfig, nil, &c.limitA)
